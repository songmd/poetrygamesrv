...
class RhesisContestConsumer(AsyncJsonWebsocketConsumer):
    ...
    #   开始或者加入游戏
    @database_sync_to_async
    def db_start_or_join_game(self, user, level, room_type):
        ...
        with transaction.atomic():
            ...
            room = GameRoom.objects.select_for_update().filter(Q(player1=None) | Q(player2=None), for_friend=False,
                                                               level=level, type=room_type).first()
            if room is None:
                room = GameRoom.objects.create(level=level, type=room_type)
    ...
    def db_start_game(self, user):
        ...
        with transaction.atomic():
            ...
            room = GameRoom.objects.create(for_friend=True, level=player.user.level, type=0)
            room.select_questions()
            room.player1 = player
            player.user.add_question_id(room.get_questions_id())
            room.save()
    ...
    def db_join_game(self, user, friend):
        ...
        with transaction.atomic():
            ...
            room = GameRoom.objects.select_for_update().filter(player1__user__id=friend, player2=None, type=0).first()
            if room is None:
                return False
