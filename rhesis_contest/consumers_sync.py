...
class RhesisContestConsumer(JsonWebsocketConsumer):
    # 此consumer所代表的玩家信息
    _player_info = dict()
    ...
    def receive_json(self, content, **kwargs):
        ...
        if content['type'] == 'start_or_join':
            ...
            success = self.db_start_or_join_game(content['user'], int(content.get('level', 1)))
            if not success:
                self.send_json(dict(err_code=-1, type='error'), close=True)
        ...
        if content['type'] == 'continue':
            ...
            print('continue game')
            success, start_game = self.db_continue_game()
            if not success:
                self.send_json(dict(err_code=-1, type='error'), close=True)
        ...
        if content['type'] == 'start':
            success = self.db_start_game(content['user'])
            if not success:
                self.send_json(dict(err_code=-1, type='error'), close=True)

        # 进入好友的游戏
        if content['type'] == 'join':
            success = self.db_join_game(content['user'], content.get('friend'))
            if not success:
                self.send_json(dict(err_code=-1, type='error'), close=True)
        ...
        if content['type'] == 'report':
            success, turn_result = self.db_report_answer(content.get('turn'),
                                                         content.get('answer'),
            ...
            if turn_result is not None:
                self.send_report_event(turn_result)
                self.send_report_response(turn_result)

    #   发送给对手consumer游戏开始的通知
    def send_start_event(self):
        print('send start event')
        async_to_sync(self.channel_layer.send)(self._rival_channel_name,
                                               dict(type='start.event',
    ...
    def send_report_event(self, turn_result):
        # 对手是模拟的，无需通知
        if self._rival_channel_name is None:
            return
        async_to_sync(self.channel_layer.send)(self._rival_channel_name,
    ...
    #   发送游戏回合报告的应答
    def send_report_response(self, turn_result):
